...
// firmware files - the .bin format will be tried first, then .hex
#define USBDUXFAST_FIRMWARE_BIN		"usbduxfast_firmware.bin"
#define USBDUXFAST_FIRMWARE_HEX		"usbduxfast_firmware.hex"
...
static void usbduxfast_firmware_upload_common(usbduxfastsub_t * usbduxfastsub,
	uint8_t * firmwareBinary, unsigned int sizeFirmware)
{
	struct usb_device *usbdev = usbduxfastsub->usbdev;
	int ret;

	// upload the binary firmware
	ret = firmwareUpload(usbduxfastsub, firmwareBinary, sizeFirmware);
	...
	comedi_usb_auto_config(usbduxfastsub->interface, &driver_usbduxfast,
			       (unsigned long)usbduxfastsub);
}

static void usbduxfast_firmware_request_complete_handler_hex(
	...
	void *context)
{
	...
	usbduxfast_firmware_upload_common(usbduxfastsub_tmp, bin_firmware,
					  bin_size);
...
static void usbduxfast_firmware_request_complete_handler_bin(
	const struct firmware *fw,
	void *context)
{
	usbduxfastsub_t * usbduxfastsub_tmp = (usbduxfastsub_t *)context;
	...
	if (fw == NULL) {
		...
		ret = request_firmware_nowait(THIS_MODULE,
					      FW_ACTION_HOTPLUG,
					      USBDUXFAST_FIRMWARE_HEX,
					      &usbdev->dev,
					      GFP_KERNEL,
					      usbduxfastsub_tmp,
					      usbduxfast_firmware_request_complete_handler_hex);
		if (ret) {
			dev_err(&usbdev->dev,
	...
	// continue firmware upload and free the allocated buffer
	usbduxfast_firmware_upload_common(usbduxfastsub_tmp, bin_firmware, fw->size);
	...
	const struct usb_device_id *id)
{
	...
	ret = request_firmware_nowait(THIS_MODULE,
				      FW_ACTION_HOTPLUG,
				      USBDUXFAST_FIRMWARE_BIN,
				      &udev->dev,
				      GFP_KERNEL,
				      usbduxfastsub + index,
				      usbduxfast_firmware_request_complete_handler_bin);
...
static int usbduxfast_attach_common(comedi_device * dev)
{
	int ret;
	comedi_subdevice *s = NULL;
	usbduxfastsub_t *usbduxfastsub_tmp = dev->private;

	// pointer back to the corresponding comedi device
	usbduxfastsub_tmp->comedidev = dev;
	...
	usbduxfastsub_tmp->attached = 1;

	printk("comedi%d: usbduxfast: usb-device %s is attached to comedi.\n",
		dev->minor, dev_name(&usbduxfastsub_tmp->interface->dev));
...
static int usbduxfast_attach(comedi_device * dev, comedi_devconfig * it)
{
	...
	for (i = 0; i < NUMUSBDUXFAST; i++) {
		usbduxfastsub_tmp = &usbduxfastsub[i];
		mutex_lock(&usbduxfastsub_tmp->mutex);
		if (usbduxfastsub_tmp->probed && !usbduxfastsub_tmp->attached) {
			break;
		}
		mutex_unlock(&usbduxfastsub_tmp->mutex);
	}
	...
	dev->private = usbduxfastsub_tmp;
	ret = usbduxfast_attach_common(dev);
	mutex_unlock(&usbduxfastsub_tmp->mutex);
...
static int usbduxfast_auto_attach(comedi_device * dev, unsigned long context)
{
	usbduxfastsub_t *usbduxfastsub_tmp = (usbduxfastsub_t *)context;
	int ret;

	if (!usbduxfastsub_tmp) {
		printk("comedi%d: usbduxfast: error: auto-attach failed, no context.\n",
			dev->minor);
		return -ENODEV;
	}
	...
	mutex_lock(&start_stop_mutex);
	mutex_lock(&usbduxfastsub_tmp->mutex);
	if (!usbduxfastsub_tmp->probed) {
		printk("comedi%d: usbduxfast: error: auto-attach failed, usb-device not probed.\n",
			dev->minor);
		ret = -ENODEV;
		goto out;
	}
	if (usbduxfastsub_tmp->attached) {
		printk("comedi%d: usbduxfast: error: auto-attach failed, usb-device already attached.\n",
			dev->minor);
		ret = -EBUSY;
		goto out;
	}
...
static int init_usbduxfast(void)
{
	...
	init_usb_devices();
	ret = usb_register(&usbduxfastsub_driver);
	if (ret < 0) {
		goto uninit_usb;
	}
	ret = comedi_driver_register(&driver_usbduxfast);
	if (ret < 0) {
		goto unregister_usb;
	}
	return 0;
...
MODULE_FIRMWARE(USBDUXFAST_FIRMWARE_BIN);
